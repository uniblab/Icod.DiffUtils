...
$stageDirectoryName = "Icod.DiffUtils-$Version-$RuntimeIdentifier"
...
$projects = [ordered]@{
    'cmp' = 'cmp/Icod.DiffUtils.Cmp.csproj'
    'diff' = 'diff/Icod.DiffUtils.Diff.csproj'
    'diff3' = 'diff3/Icod.DiffUtils.Diff3.csproj'
    'sdiff' = 'sdiff/Icod.DiffUtils.SDiff.csproj'
    'diffutil' = 'diffutil/Icod.DiffUtils.DiffUtil.csproj'
}
...
try {
    ...
    if ($RuntimeIdentifier -eq $currentRid) {
        foreach ($commandName in $projects.Keys) {
            ...
            $expectedOutput = if ('diffutil' -eq $commandName) {
                "diffutil (Icod.DiffUtils) $Version"
            } else {
                ''
            }
